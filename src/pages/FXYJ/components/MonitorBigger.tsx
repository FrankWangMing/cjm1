...
export const MonitorBigger = observer(() => {
    ...
    (): {
      ...
      async cameraControl(index: number, command: string) {
        await MonitorServer.controlCruise(
          Number(SLYZTStore.modalData.id),
          command,
          index
        );
      },
  ...
  const controlPTZ = async (ptzCmd: string) => {
    let cameraId = SLYZTStore.modalData.id as number;
    await MonitorServer.controlMonitorPTZById(0, cameraId, ptzCmd);
    await MonitorServer.controlMonitorPTZById(1, cameraId, ptzCmd);
  };

  useUpdateEffect(() => {
    if (SLYZTStore.modalData.id && SLYZTStore.modalData.id != -1)
      getMonitorById(SLYZTStore.modalData.id as number);
  }, [SLYZTStore.modalData.id]);
    ...
    <Fragment>
      <div className="flex-between cus-modal-header">
        ...
        <div className="flex-center">
          ...
          <CloseOutlined
            onClick={() => {
              SLYZTStore.modalData.type = '';
              SLYZTStore.modalData.isVisible = false;
              SLYZTStore.modalData.style = null;
            }}
            alt="关闭"
          />
        </div>
        ...
        <div className="monitor-content-outer" id="monitor-content-outer">
          <div
            ...
            <img
              ...
              src="http://10.0.1.66:8051/api/resource/dynamicDir/pdf/videopictures/中洲镇乘风源村乘风源村.jpg"></img>
          </div>
                    ...
                    return (
                      <div
                        ...
                        className={[
                          ...
                          SLYZTStore.modalData.id == item.cameraId
...
const MonitorBiggerWrapper = styled.div<{ isExpand: boolean }>`
  ...
  .monitor-content-outer {
    ...
    .monitor-content-playlist {
      ...
      .monitor-content-playlist-item:nth-child(1) {
        margin-top: 4rem;
      }
      .monitor-content-playlist-item_selected,
      ...
      .monitor-content-playlist_header:hover {
        background: rgba(149, 174, 255, 0.28) !important;
      }
    }
    /* .border-line {
      width: 1rem;
    ...
    .control {
      width: 38rem;
      height: 38rem;
      background: linear-gradient(270deg, #1564b3 0%, #15b9ff 100%);
      cursor: pointer;
      font-size: 25rem;
      color: #fff;
      text-align: center;
